...
export default function LimpezaPage() {
  ...
  async function finalizarLimpeza(roomId: string) {
    ...
    await supabase.from('housekeeping_records').insert({
      property_id: PROPERTY_ID,
      room_id: roomId,
      staff_id: attendantId,
      cleaning_done: true,
      maintenance_needed: false,
      verified_by: attendantId,
    })
  ...
  async function reportarAvaria(roomId: string) {
    if (!issueDescription.trim() || !attendantId) return
    setSaving(true)
    const supabase = createClient()
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="card space-y-2">
        <h2 className="text-xs font-bold text-ink-muted uppercase tracking-wide">Quem está a fazer o serviço?</h2>
        ...
        {!attendantId && (
          <p className="text-xs text-amber-700">Seleccione um funcionário para poder registar ações nesta página.</p>
        )}
      </div>
                ...
                <div className="space-y-2 pt-2 border-t border-border">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={saving || !issueDescription.trim() || !attendantId}
